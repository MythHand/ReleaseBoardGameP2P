...
export default function Lobby({
  ...
  role = 'host', // host | guest — пока для вида
  ...
  const [menuFor, setMenuFor] = useState(null) // id строки с открытым меню «…»
  const [menuHint, setMenuHint] = useState('') // подсказка по задизейбленному пункту
  ...
  const kickSpectator = (id) => {
    setSpectators((ss) => ss.filter((s) => s.id !== id))
    setMenuFor(null)
  }
  const toSpectator = (id) => {
  ...
  // меню «⋯» хоста: items = [{ label, danger?, disabled?, hint?, onClick }]
  ...
  return (
    <div className={styles.lobby}>
        ...
        <section className={styles.players}>
          <div className={styles.scrollArea}>
            ...
            <ul className={styles.list}>
              ...
              {spectators.length === 0 && (
                <li className={styles.slotEmpty}>пока без зрителей</li>
              )}
            </ul>
          </div>
          ...
          <div className={styles.actions}>
            {isHost ? (
              <Button disabled={!canStart}>начать игру</Button>
            ) : (
      ...
      <Modal
        ...
        title="Расформировать лобби?"
      >
        <p className={styles.confirmText}>
          Лобби будет закрыто, все подключённые игроки — отключены. Действие нельзя
          отменить.
        </p>
