...
import { buildSequence } from './lines.js'
import LoaderAudio from './audio.js'
import LogoSvg from './Logo.jsx'
import './boot.css'

// ---------------- Terminal ----------------
function Terminal({ active, onComplete }) {
...
// ---------------- Logo stage ----------------
function LogoStage({ active, onComplete }) {
  ...
  useEffect(() => {
    ...
    const timers = []
    let raf = 0
    const T = (fn, ms) => timers.push(setTimeout(() => !cancelled && fn(), ms))
...
// ---------------- Restart screen ----------------
function RestartScreen({ onRestart }) {
