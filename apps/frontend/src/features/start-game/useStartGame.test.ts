...
// They must be the very ones the lobby rows showed — the host-seeded pick —
// so a bot does not get renamed on its way to the board.
it('builds one name per effective bot, in seat order, from the host-seeded pick', () => {
  session = {
    gameId: null,
    startGame,
    state: {
  ...
  const { result } = renderHook(() => useStartGame())
  result.current()
  expect(startGame).toHaveBeenCalledWith(botNames('host', 3))
})
