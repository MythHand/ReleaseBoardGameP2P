...
- **All visuals come from `@release/ui`.** Pages add layout via co-located `*.module.css` files only — never new visual components.
- **All text goes through `<Typography>` from `@release/ui`** — render copy with `<Typography>`, never a raw text element (`<p>` / `<span>` / `<h1>`…) and never hand-written font/size/tracking declarations or `composes` from the typography scale in module CSS. Full rule: [apps/ui/CLAUDE.md](../ui/CLAUDE.md#typography-rule).
- **Colors are design tokens only.** Never hardcode a color value — no `#hex`, `rgb()/hsl()`, named colors. Use `var(--*)` tokens from `apps/ui/src/design/tokens.css` in module CSS. Missing a color → add a token there, don't inline a raw value.
- **Every form must use `<Form>` from `~/shared/ui/Form`.** Never use a raw `<form>` element — `Form` provides noValidate, required-field validation with error display via `FormCtx`, and a typed `onSubmit(data: Record<string, string>)` callback. Use `<FormField>` (not `<Input>`) for fields inside a form so they receive context-driven errors automatically.
