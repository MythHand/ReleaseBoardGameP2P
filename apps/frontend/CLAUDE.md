...
- **All text goes through `<Typography>` from `@release/ui`** — render copy with `<Typography>`, never a raw text element (`<p>` / `<span>` / `<h1>`…) and never Tailwind font/size/tracking utilities on text. Full rule: [apps/ui/CLAUDE.md](../ui/CLAUDE.md#typography-rule).
- **Colors are design tokens only.** Never hardcode a color value — no `#hex`, `rgb()/hsl()`, named colors, or Tailwind arbitrary values (`text-[#fff]`, `bg-[rgb(...)]`). Use token-backed utilities (`text-brand-green`, `bg-surface-1`, `text-cat-release`) that resolve through the `@theme` bridge in `src/app/index.css` to `apps/ui/src/design/tokens.css`. Missing a color → add a token there and bridge it in `@theme`, don't inline a raw value.
