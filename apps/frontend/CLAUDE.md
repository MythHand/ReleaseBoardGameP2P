...
- **All text goes through `<Typography>` from `@release/ui`** — render copy with `<Typography>`, never a raw text element (`<p>` / `<span>` / `<h1>`…) and never raw font/size/tracking declarations outside `composes` from the shared typography scale. Full rule: [apps/ui/CLAUDE.md](../ui/CLAUDE.md#typography-rule).
