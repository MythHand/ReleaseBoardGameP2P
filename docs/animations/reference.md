...
| `ai` | `features/board-beats/aiBeat.tsx` (`run`/`runTaken`/`runRefused`) | `aiEvent` — the trigger off its own pile to `cause`, the card the events deck gives up to `effect`, both held `TABLE_HOLD` (doubled, `HALLUCINATION_HOLD`, for `ai-hallucination`), then one of `AiTail`'s six endings (`zone`/`crush`/`turnEnded`/`alarm`/`standing`/`none`) read off what follows in the batch, never re-derived from the card's own id; the plan behind it reads two facts outside its own batch — `releaseEventsOf` (a crush's destination, events deck or discard) and `owed` (the live pending, to tell a standing prompt from an unanswered mimic). `takenFromDiscard` — `ai-inside`'s answer, shown open at the centre for the whole table before it splits by `mine`; `crushRefused` — a Crush its owner passed on (owner, 24.09): the `crush` ending's own release flight (`raiseCrushed`/`sendCrushed`, shared with `run`) and the standing AI card's road home (`leaveTheStanding`, shared with `runTaken`). The mimic's glow is lit by the runner once its card is face up (`BeatRun.raiseAlarm`), not by the beat as a whole; the trigger's pile gives the card up as it takes off (`offThePile`) | `drawToCenter` (×2, via `toSlot`), `flipCard` (via `patch`), `playToReleaseZone`, `returnToDeck`, `dealToSeat`, the discard-exit step |
...
| `CardCatalog` | `@/table/CardCatalog` | `<CardCatalog cards open selected? chosen? onPick? onDrop? width? stagger? previewRoot? previewSelected? />` — the set of face-up cards you name one from. Cells appear with a stagger and GROW on hover (they are there to be read, not lifted); `open` means the choice is on, `chosen` holds the named one enlarged while the rest slide away, `selected` is armed-but-not-committed. Confirmation lives outside it (usually `ConfirmAction` — naming a card is irreversible). It fills the area it is given; where that area is belongs to the screen. A scrolling consumer supplies `previewRoot`, a positioned non-scrolling reading layer: the enlarged hover/focus or chosen copy is clamped there with room for its glow, without intercepting neighbouring choices. Scrolling clears a hover preview. |
