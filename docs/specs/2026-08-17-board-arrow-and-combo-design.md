...
> - **Складывание пары стало шагом словаря — `usePairFold`.** Последовательность «поднять носитель с
>   `CardPair` → покрасить обеим половинам входные позы → `nextFrames` → параллельный `foldIntoPair`
>   на каждую», описанная здесь как ход, была написана заново четырежды. Плейграундные сцены (`Combo`,
>   `Defense Release`) переведены на шаг; три копии на борде заменяются вызовом.
> - Шаг закрыл и слепое пятно флаерной формы: пара монтируется невидимой и открывается в тот же тик,
>   когда половинам проставлены входные позы, поэтому кадра «пара уже сложена» больше нет.
