...
> - **Места центра стола — общий источник.** Высота ряда, сдвиги, ширины и слои живут в
>   `apps/ui/src/table/TableCentre/centre.ts` (`CENTRE_TOP` / `CENTRE_SLOTS` / `CENTRE_SETS`), а не в
>   CSS сцены и борда по отдельности; сцена `Defense Release` берёт их оттуда.
> - **Строка «чего стол ждёт» стала компонентом кита** — `AskLine`, висит на высоте центра, со своей
>   страницей `Ask line`. Копия борда осталась.
> - **Складывание защиты со своим судо идёт через шаг `usePairFold`.**
> - **Посадка карты на место — кривая `LAND`, а не `SNAP`.** `SNAP` осталась там, где элемент
>   ПОЯВЛЯЕТСЯ на месте (бейдж дока, тосты); у карты есть путь по столу, и на 480–620ms та же кривая
>   съедала его в первой трети — релиз в зону читался броском.
> - **Наклон приземления описан инвариантом I11**, и под него в словаре есть шаг `landInPose`:
>   разложенное системой ложится ровно, пришедшее из руки игрока — под своим углом. Борду осталось
>   позвать его для одиночной атаки.
