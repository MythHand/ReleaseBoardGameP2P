...
Every question here is a place where [`docs/rules-board-game.md`](../rules-board-game.md) leaves room and the engine chose for it, by accident of implementation rather than by decision. Answering **"keep current behaviour"** is a perfectly good answer — the point is that it stops being an accident.
...
- [ ] **1. Can DDoS be answered by a defence card?**
  ...
  Hotfix, Rubber Ducky and PR Approved name their targets and exclude DDoS ([rules :110](../rules-board-game.md)). Rollback ([:114](../rules-board-game.md)), Not a Bug ([:118](../rules-board-game.md)) and Works on my Machine ([:122](../rules-board-game.md)) all read "Отменяет атаку" with no card list — so they either cover DDoS or they do not, and the text does not say which.
...
- [ ] **2. Can DDoS be thrown at a freshly played Code Review-protected release?**
  ...
  "DDoS — единственная атака, работающая против защищённого релиза" ([:59](../rules-board-game.md)). Code Review can only ever be attached as the release is played, so that sentence reads like it is about the moment of play.
...
- [ ] **3. Which release does Security Bug need you to have before the stolen one is discarded instead?**
  ...
  "если у вас выложен Release в зоне релиза, атакованный Release отправляется в сброс" ([:100](../rules-board-game.md)) reads as *any* release. The engine reads it as *that same slot* ([`attacks.ts:43`](../../packages/engine/src/fake/attacks.ts)): the steal happens unless the thief's matching slot is already occupied.
...
- [ ] **5. Is Monitoring's protection automatic, or one option among several?**
  ...
  "угроза от них игнорируется" ([:86](../rules-board-game.md)) reads automatic. The engine offers it as one neutralize method beside Debugger and sacrifice ([`triggers.ts:19`](../../packages/engine/src/fake/triggers.ts)), so a player holding both Monitoring and a Debugger can be prompted into spending the Debugger.
...
- [ ] **6. Where do an eliminated player's cards go?**
  ...
  The rules say nothing ([:177](../rules-board-game.md)). The engine sends hand and zone to the discard.
...
- [ ] **7. Who starts?** *(technical — no rules answer needed)*
  ...
  The rules give a social rule ([:20](../rules-board-game.md)); the engine seats the host first. Recorded only so the difference is written down somewhere.
...
The reflection branch turns an attack back on whoever threw it. "Эффект карты атаки оборачивается против самого атакующего" ([:122](../rules-board-game.md)) — three things follow from "эффект" that the engine currently decides on its own.

- [ ] **9. Does a reflected attack get through Code Review?**

  Code Review makes a release invulnerable to Bug, Out of Memory, Legacy Code and Security Bug, "даже с sudo-усилением" ([:132](../rules-board-game.md)). The engine's reflection picks the attacker's first occupied slot without checking for protection, and the Code Review attached to it is discarded along with the release.
...
- [ ] **10. A reflected Security Bug — does the reflector take the release, or is it destroyed?**
  ...
  Security Bug's own effect is "забираете карту релиза в свою зону релиза" ([:100](../rules-board-game.md)). Reflected, the engine destroys the attacker's release rather than moving it.
